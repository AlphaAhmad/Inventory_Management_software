...
class Repository:
    def __init__(self):
        ...
        # Probe once: does phone_details have the serial_number column?
        # Avoids write failures until the optional migration is run.
        self._has_serial_column: bool | None = None

    def _serial_column_available(self) -> bool:
        if self._has_serial_column is None:
            try:
                self.client.table("phone_details").select("serial_number").limit(1).execute()
                self._has_serial_column = True
            except Exception:
                self._has_serial_column = False
        return self._has_serial_column
    ...
    def create_phone_details(self, details: PhoneDetails) -> PhoneDetails:
        ...
        if self._serial_column_available():
            data["serial_number"] = details.serial_number
    ...
    def search_products(
        ...
        # Also search by IMEI in phone_details (imei1 or imei2)
        imei_resp = (
            self.client.table("phone_details")
            .select("product_id")
    ...
    def get_dashboard_stats(self) -> dict:
        ...
        # Stock value = cost basis of on-hand inventory (what the shop paid)
        # Only include products with status "in_stock" AND quantity > 0 (exclude sold, claimed, zero qty, etc.)
        in_stock_rows = [r for r in rows if r["status"] == "in_stock" and (r["quantity"] or 0) > 0]
        total_value = sum((r["purchase_price"] or 0) * (r["quantity"] or 0) for r in in_stock_rows)
        low_stock = sum(
            1 for r in rows
    ...
    def get_total_stock_value(self) -> float:
        resp = self.client.table("products").select("purchase_price, quantity, status").eq("status", "in_stock").execute()
        return sum(row["purchase_price"] * row["quantity"] for row in resp.data if row["quantity"] > 0)
