...
class BrandedAccessoriesPage(QWidget):
    ...
    def _setup_ui(self):
        ...
        add_category_btn.clicked.connect(self._on_add_product_type)
        filter_row.addWidget(add_category_btn)

        remove_category_btn = QPushButton("Remove Category")
        remove_category_btn.setObjectName("dangerBtn")
        remove_category_btn.clicked.connect(self._on_remove_product_type)
        filter_row.addWidget(remove_category_btn)
    ...
    def _load_product_types(self):
        """Reload the product types dropdown."""
    ...
    def _on_add_product_type(self):
        ...
        name, ok = QInputDialog.getText(self, "Add Category", f"Enter category name for {self.brand_name}:")
        if ok and name.strip():
            try:
                pt = ProductType(subcategory_id=self.subcategory_id, name=name.strip())
                self.inventory_service.create_product_type(pt)
                self._load_product_types()
                self.filter_combo.setCurrentText(name.strip())
                QMessageBox.information(self, "Success", f"Category '{name.strip()}' added successfully!")
    ...
    def _on_remove_product_type(self):
        ...
        if selected == "All Product Types":
            QMessageBox.warning(self, "Invalid Selection", "Please select a specific category to remove.")
            return

        reply = QMessageBox.question(
            self, "Confirm Delete",
            f"Are you sure you want to delete '{selected}' category?\nThis will delete all products in this category.",
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                product_types = self.inventory_service.get_product_types(self.subcategory_id)
                pt = next((p for p in product_types if p.name == selected), None)
                if pt:
                    self.inventory_service.delete_product_type(pt.id)
                    self._load_product_types()
                    self.filter_combo.setCurrentIndex(0)
                    self.refresh_data()
                    QMessageBox.information(self, "Success", f"Category '{selected}' deleted successfully!")
            except Exception as e:
