...
class AccessoriesPage(QWidget):
    def __init__(self, parent=None):
        ...
        self.category = self.inventory_service.get_category_by_name("Accessories")
        self.category_id = self.category.id if self.category else ""

        # Find the "General Accessories" subcategory
        self.subcategory = None
        if self.category_id:
            subcategories = self.inventory_service.get_subcategories(self.category_id)
            self.subcategory = next(
                (s for s in subcategories if s.name == "General Accessories"), None
            )
    ...
    def _setup_ui(self):
        ...
        add_type_btn = QPushButton("Add Accessory Type")
    ...
    def _on_add_product_type(self):
        ...
        name, ok = QInputDialog.getText(self, "Add Accessory Type", "Enter accessory type name (e.g. Smart Watch):")
        if ok and name.strip():
            try:
                ...
                attrs = [
                    AttributeDefinition(product_type_id=created_pt.id, name="compatible_brand", label="Compatible Brand", field_type="text", display_order=1),
                ...
                QMessageBox.information(self, "Success", f"Accessory Type '{name.strip()}' added successfully!")
            except Exception as e:
                QMessageBox.critical(self, "Error", f"Failed to add accessory type: {e}")
