...
class CasesPage(QWidget):
    ...
    def _on_remove_subcategory(self):
        selected = self.filter_combo.currentText()
        if selected == "All Subcategories":
            QMessageBox.warning(self, "Invalid Selection", "Please select a specific subcategory to remove.")
            return

        reply = QMessageBox.question(
            self, "Confirm Delete",
            f"Are you sure you want to delete '{selected}' subcategory?\nThis will delete all products in this subcategory.",
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                ...
                if sub:
                    self.inventory_service.delete_subcategory(sub.id)
                    self._load_subcategories()
                    self.filter_combo.setCurrentIndex(0)
                    self.refresh_data()
                    QMessageBox.information(self, "Success", f"Subcategory '{selected}' deleted successfully!")
            except Exception as e:
